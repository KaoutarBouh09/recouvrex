...
import {
  getCurrentDate,
  getCurrentDateTime,
} from "src/utils/formatDate/CurrentDateTime";
import AddTwoToneIcon from "@mui/icons-material/AddTwoTone";
import { UserContext } from "src/contexts/UserContext";
import { createNewClient } from "src/utils/api/client/ClientApi";
...
export default function NewClient({ setIsNewFactOpen }) {
  //use context
  ...
  const [tiersType, setTiersType] = useState("CUSTOMER_PP");
  const [title, setTitle] = useState("titleTest");
  const [lastName, setLastName] = useState("kifah");
  const [firstName, setFirstName] = useState("Omar");
  const [companyName, setCompanyName] = useState("Naritaware");
  const [birthDate, setBirthDate] = useState("2000-10-10");
  const [nationality, setNationality] = useState("Marocaine");
  const [countryOfResidence, setCountryOfResidence] = useState("Morocco");
  const [businessSector, setBusinessSector] = useState("SARL");
  const [legalForm, setLegalForm] = useState("SARL");
  const [occupation, setOccupation] = useState("Ingenieure");
  const [personalEmail, setPersonalEmail] = useState("frovoibifriwau-7756@yopmail.com");
  const [businessEmail, setBusinessEmail] = useState("frovoibifriwau-7756@yopmail.com");
  const [privatePhone, setPrivatePhone] = useState("064433");
  const [businessPhone, setBusinessPhone] = useState("36478");
  const [landLinePhone, setLandLinePhone] = useState("092233");
  const [faxNumber, setFaxNumber] = useState("121314");
  const [commercialRegister, setCommercialRegister] = useState("EDRF88821");
  const [supportingDocumentType, setSupportingDocumentType] = useState("CIN");
  const [supportingDocumentNumber, setSupportingDocumentNumber] = useState("2");
  const [
    supportingDocumentExpirationDate,
    setSupportingDocumentExpirationDate,
  ] = useState("2030-10-12");
  const [maritalStatus, setMaritalStatus] = useState("Marié");
  ...
  const getCaseInfo = async (caseId) => {
    try {
      const result = await getFilteredCasesByCaseId(caseId);
      //  setCaseInfo(result);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSuccessMessage("Le nouvel client a été créé avec succès!");
      setErrorMessage("");
      setTimeout(() => {
        setOpen(false);
        setSuccessMessage("");
    ...
    <React.Fragment>
      ...
      >
        <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
          <Typography variant="h3">Créer nouveau client</Typography>
        </DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
