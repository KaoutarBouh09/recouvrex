...
interface ClientFormProps {
  excelData: ThirdParty[] | null;
  setExcelData: React.Dispatch<React.SetStateAction<ThirdParty[] | null>>;
}

const ClientForm: React.FC<ClientFormProps> = ({excelData, setExcelData}) => {
  ...
  const insertManyThirdpartiesTest = async () => {
    ...
    try {
      ...
      console.log("Created clients 💛💚:", createdThirdParties);
    
      handleShowSnackbar({
        message: "Les informations des clients ont été enregistrées avec succès.",
        severity: "success",
      })
    } catch (error) {
      ...
      handleShowSnackbar({
        message: "Malheureusement, les informations des clients n'ont pas été enregistrées!",
        severity: "error",
      })
  ...
  return (
    ...
    {  <Paper
      ...
      >
        ...
        Faites glisser et déposez votre fichier Excel des clients dans cette zone pour le télécharger.
        </Typography>
        <input
          type="file"
          accept=".xlsx, .xls"
    ...
    { excelData && <ThirdPartyTable thirdParties={excelData} />}
      {excelData && (
        <>
        <Button
         sx={{ mr: 1,my:1}}
       startIcon={!isLoading && <CloudUploadIcon />}
        ...
        >
    { isLoading?  <TextLoading /> : "Ajouter les clients"}
        </Button>
