...
import { deleteDueDateById, getDueDatesByCaseId, getDueDatesByCreditId } from "src/utils/api/dueDate/DueDateApi";
...
import { useDueDate } from "src/contexts/DueDateContext";

export default function InvoicesTable() {
    //context states
    const { creditData, setCreditData } = useCredit();
  const { selectedCase } = useContext(CaseContext);
  const { dueDateData, setDueDateData } = useDueDate();
  ...
  const getDueDatesInfo = async (id: number) => {
    try {
      const result = await getDueDatesByCreditId(id);
      setDueDates(result);
  ...
  useEffect(() => {
    getDueDatesInfo(creditData.id);
    
  }, [creditData, isNewFactOpen]);
        ...
        <TableHead>
          <TableRow>
            ...
            <TableCell align="right">
              <NewFacture setIsNewFactOpen={setIsNewFactOpen}/>
            </TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {dueDates.map((dueDate) => (
            ...
            >
              ...
              <TableCell component="th" scope="row">
                <Link to={`/dueDate/${dueDate.dueDateId}`} rel="noopener noreferrer" onClick={()=>setDueDateData(dueDate)}>
                  {dueDate.dueDateId}
                </Link>
         ...
         />
              ...
              <TableCell align="right">
                <Tooltip title="Edit Order" arrow>
                  ...
                  */}
              <EditInvoice check={false} dueDate={dueDate} setIsNewFactOpen={setIsNewFactOpen}/>
