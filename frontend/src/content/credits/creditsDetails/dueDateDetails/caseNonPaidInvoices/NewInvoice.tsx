...
export default function NewFacture({ setIsNewFactOpen }) {
  //use context
  ...
  const [clientFirstname, setClientFirstname] = useState(clientData.firstName);
  const [clientLastname, setClientLastname] = useState(clientData.lastName);
  ...
  const [errorSendingEmailOpen, setErrorSendingEmailOpen] = useState(false);
  // useEffect(() => {
  //   getCaseInfo(caseId);
  // }, [caseId]);
  ...
  const sendEmailNotification = async () => {
    try {
      await sendEmailToClient(
        ...
        `Bonjour Monsieur ${clientFirstname},Vous avez une facture d'un montant principal de ${principalAmount} DH.
        Pour plus d'informations, vous pouvez contacter le service Recouvrex ou vous rendre à l'agence la plus proche.
        Cordialement,`
      );
    ...
    <React.Fragment>
      ...
      >
        <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
          <Typography variant="h3">Créer Nouvelle Facture</Typography>
        </DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
            ...
            <Grid item xs={4}>
              <TextField
                ...
                onChange={(e) => {
                  ...
                  selectedDate1>currentDate?setDueDateStatus("EN_ATTENTE"):setDueDateStatus("IMPAYEE");
                  console.log("🦍🦍selected date : ",selectedDate1 , "current date : ", currentDate)
                }}
      ...
      <Dialog
          ...
          >
           <IconButton
          aria-label="close"
          onClick={()=>{setErrorSendingEmailOpen(false)}}
          sx={{
            position: "absolute",
            right: 8,
            top: 8,
            ...
            <DialogContent>
              ...
              >
                {errorSendingEmail ? 
                  ...
                  variant="outlined"
                  severity="warning"
                  sx={{ width: "500px", margin: "auto" , marginTop:5 }}
                >
                  error de l'envoie d'email au client , veuillez verifier address email du client
                </Alert> : 
                  <Alert
                  variant="outlined"
                  severity="success"
                  sx={{ width: "500px", margin: "auto" , marginTop:5 }}
                >
                   une notification email a ete envoyer au client avec succees
                </Alert> }
